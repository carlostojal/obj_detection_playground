...
class TestPadding(unittest.TestCase):
    ...
    def test_unpad_bboxes(self):
        img = torch.rand(1, 3, 1000, 2000)
        bboxes = torch.tensor([[[0.1, 0.1, 0.1, 0.1, 0], [0.2, 0.2, 0.2, 0.2, 1]]])
        target_size = torch.tensor([[480, 640]])
        padded_img, padded_bboxes, padding = pad_image(img, bboxes, target_size)
        unpadded_bboxes = unpad_bboxes(padded_bboxes, torch.tensor([[480, 640]]), padding)
        self.assertEqual(unpadded_bboxes.size(), (1, 2, 5))
        self.assertEqual(unpadded_bboxes[0, 0, 0], 0.1)
        self.assertEqual(unpadded_bboxes[0, 0, 1], 0.1)
        self.assertEqual(unpadded_bboxes[0, 0, 2], 0.1)
        self.assertEqual(unpadded_bboxes[0, 0, 3], 0.1)
